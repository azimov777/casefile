...
async def list_issues(
    ...
    Простое перечисление с единственным сужением — по очереди. Отбор по статусу,
    исполнителю, значениям полей и язык запросов живут в `GET /search/issues`: второй
    набор фильтров здесь разошёлся бы с поиском на первом же краевом случае.
    """
...
async def read_issue(
    ...
    """Задача одним объектом: системные поля, значения кастомных полей и версия.

    Связанные объекты приезжают ссылками — очередь ключом, статус и тип ссылками
    справочника, акторы ключами. Полные записи отдают конфигурация очереди и список
    акторов: рассылать их с каждой задачей значило бы слать одно и то же по многу раз.

    Всё, что рисуется на экране задачи рядом с ней — переходы, связи, обсуждение,
    чеклист, — отдаёт `GET /issues/{issue_key}/card` одним запросом.
    """
