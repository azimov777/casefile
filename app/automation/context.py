...
class RuleContext:
    ...
    def target(self) -> Issue:
        """Задача правила. Обращение при её отсутствии — ошибка программиста, не данных.

        Отдельное свойство рядом с необязательным полем нужно ровно затем, чтобы
        правило, написанное для задачи, падало понятной ошибкой, а не `AttributeError`
        на `None` где-то в третьей строке действия.
        """
