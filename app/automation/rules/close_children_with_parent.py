...
Условие правила спрашивает **событие**, а действие работает с актуальными подзадачами —
это общий контракт триггеров, и здесь он виден целиком (см. шапку
`app/automation/context.py`). Написанное по `ctx.target.status` условие означало бы
столько срабатываний, сколько событий успело накопиться в очереди до старта воркера.
"""
...
async def close_children_with_parent(ctx: RuleContext) -> None:
    ...
    if not await ctx.entered_category(StatusCategory.DONE):
        # Вопрос задаётся событию, а не строке задачи: строка отвечает за момент
        # обработки, и на накопленной очереди условие прошло бы у каждого события,
        # став истинным для последнего. Смена статуса внутри работы подзадач не
        # касается, переход между двумя закрывающими статусами — тоже: закрытие уже
        # разобрано на том событии, которое его и принесло.
        ctx.skip("parent_not_done")

    # Дальше — действие, и оно идёт по актуальным строкам: закрывать надо те подзадачи,
    # которые открыты сейчас, а не те, что были открыты в момент события.
    parent = ctx.target
