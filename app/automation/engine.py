...
3. **Лимит срабатываний на задачу за окно.** Ловит то, что не ловят первые два: волну.
   Массовый перенос двухсот задач между статусами даёт двести событий, каждое законно,
   каждое первого уровня — глубина не растёт, реакции на себя нет. Без лимита правило
   отработает двести раз и породит двести новых событий.
...
def _in_scope(rule: AutomationRule, issue: Issue | None) -> bool:
    ...
    доски) не запускается: проверить принадлежность нечем, а «наверное,
