...
Ответ `create_task` при этом хранится ключом идемпотентности, и повтор отдаёт
**сохранённое**, а не пересобранное (`app/services/idempotency.py`): сутки после правки
в таблице лежат ответы обеих форм, и на вчерашний вызов придёт вчерашняя карточка. Это
не переходный режим в коде, а свойство хранилища, и кончается оно само — `KEY_TTL`.
