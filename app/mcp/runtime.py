...
пользуются воркер и планировщик: вышли из блока без исключения — коммит, с исключением —
откат. Своей транзакции инструмент не открывает, и это не стилистика. Ожидание
уведомлений отпускает соединение на время сна (`app/services/notifications.py`), а
собственная транзакция поверх сценария это свойство отменила бы: несколько ждущих
агентов исчерпали бы пул соединений, и встал бы весь остальной сервер.
...
к откатываемой транзакции теста. Тот же приём, что у потока событий в `app/api/deps.py`.
...
supported`), а список очередей и словарь статусов — ровно такие ресурсы. Поэтому
заголовки входящего сообщения кладёт в контекстную переменную `headers_middleware`, и
берут их оттуда и инструменты, и ресурсы — одним способом, а не двумя.
