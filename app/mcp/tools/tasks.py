...
def register(tools: Toolset) -> None:
    ...
    async def transition(
        ...
        подшитой после последнего входа в него; `in_progress → done` без положительного
        последнего вердикта по каждой проверке, подшитого после последнего входа в
        `in_progress`; вход в `in_progress` при открытом блокере; закрытие (`done` или
        `cancelled`) при незакрытых детях. В отказе — что именно мешает.

        Этих проверок нет в `transitions` у `get_task`: там таблица переходов.

        `waiting` — когда следующий ход **не твой**: человек должен ответить, посмотреть
        своими глазами, дать доступ, либо должно случиться событие вне трекера. Уходи
        туда сводкой и причиной, называющей, чего ждёшь: по ней тебя найдут отбором
        `status: waiting`, и это единственное место, где сказано, чего ждут.

        Не `waiting`, когда ждёшь другую задачу: это `blocked_by` и `open` — трекер сам
        не пустит тебя в работу, пока блокер открыт. Дождался — возвращайся в
