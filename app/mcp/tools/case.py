...
def register(tools: Toolset) -> None:
    ...
    async def add_verdict(
        ...
        на материал. «Выглядит нормально» доказательством не является. `review → done`
        требует, чтобы последний вердикт по каждой проверке был `passed`, и считает
        только вердикты текущего обзора: подшитые после последнего входа в `review`.
        Задача, вернувшаяся из `review` и пришедшая снова, проверяется заново.
        """
        async with runtime.call() as (session, actor):
            task = await tasks_service.get_task(session, key)
