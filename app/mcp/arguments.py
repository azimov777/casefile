...
Всё, что относится к одному полю, стоит в описании поля, а не в описании инструмента.

Где объявлен аргумент, решает круг его пользователей: аргумент одного инструмента — в файле
инструмента (`app/mcp/tools/<группа>/<имя>.py`), общий для нескольких инструментов группы —
в `arguments.py` группы, общий для разных групп — здесь. Правила ниже действуют на все эти
места.
...
Докстрока вложенной модели аргументов — её описание в схеме, то есть тоже метадата:
английская и короткая. Доводы разработчика стоят комментарием над классом.
