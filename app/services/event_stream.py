...
уже хранит всё и не чистится. Отсюда честная граница обещания — `stream_replay_limit`.
Клиент, отставший больше чем на этот запас, получает отказ с просьбой перечитать
состояние через API, а не молчаливый старт «с текущего момента»: тихий пропуск означал
бы клиента, который считает себя синхронизированным, не будучи им.

Если однажды появится чистка старых событий, окно переподключения обязано быть
согласовано с ней, а не назначено отдельно.
