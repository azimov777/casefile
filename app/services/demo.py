...
async def _waiting_task(
    ...
    """Задача, ждущая ответа: сценарий `CONCEPT.md`, 4.6.

    Агент упёрся в вопрос, на который сам ответить не может, задал его с признаком
    `blocking`, написал сводку и вернул задачу в `open`. Назначатель её не берёт, пока
    вопрос открыт; человек видит его в своей «входящей».
    """
    task = await tasks_service.create_task(
        session,
        actor=agent,
    ...
    await tasks_service.transition_task(
        ...
        actor=agent,
        to=TaskStatus.OPEN,
        reason="Задан блокирующий вопрос владельцу, ждать в работе нечего",
    )
    return task
