...
async def update_actor(
    ...
    *,
    ...
    поэтому отдельный признак «передано как null» здесь не нужен, и схема `ActorUpdate`
    отвергает явный `null` сама. Там, где `null` осмыслен (исполнитель, дедлайн и
    резолюция задачи), сценарий берёт общий сентинел `UNSET` из `app/core/sentinels.py`.
    """
