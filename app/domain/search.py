...
выражается выборкой всей очереди с отбором глазами (TRK-76).
"""
...
#: Прямые родители задачи в строке списка (`CONCEPT.md`, 4.4): ключ и название каждого.
#: Имя во множественном числе, потому что это список — родителей бывает несколько. Имя
#: условия отбора остаётся `parent`: `parent: TRK-7` читается «среди родителей есть
#: TRK-7», и по одному слову на каждую сторону — выдачу и отбор — путать нечего.
PARENTS_FIELD = "parents"

#: Поля, которые можно попросить в выдаче: поля карточки задачи плюс `features` и
#: `parents`. Два разных представления одной задачи в одном API — то, чего проект не
#: допускает, поэтому список строки совпадает с карточкой, признаки приходят тем же
#: вложенным объектом, что и в пакете преемника (`CONCEPT.md`, 4.2), а родители — это
#: связи `child` того же пакета, сжатые до ключа и названия.
