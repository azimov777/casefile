...
# запросов из задачи 12 разрешает эти имена в колонки, а не в `values`.
...
#
# Набор пополняется вместе с языком запросов: любое имя, которое поиск понимает сам,
# обязано лежать здесь. `status_category` — категория статуса (`new` / `in_progress` /
# `done`), `text` — псевдополе полнотекстового поиска по названию, описанию и
# обсуждению. Оба не колонки, но оба заняты поиском, и одноимённое кастомное поле было
# бы недостижимо ровно так же, как одноимённое `status`.
