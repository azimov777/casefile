"""Запись дела: неизменяемая страница журнала задачи или проекта.
...
## Владелец — задача или проект

Дело есть у задачи и у проекта (`CONCEPT.md`, 3.4), и таблица записей у них одна:
лента, сквозной `seq`, оповещение ждущих и триггер неизменяемости общие, а второй
журнал концепция запрещает (4.1). Владелец записи — `task_id` **или** `project_id`,
ровно один непустой: это держит проверка `ck_entries_one_owner`, а не дисциплина
вызывающего. Номер `no` считается внутри владельца, поэтому уникальностей две —
`(task_id, no)` и `(project_id, no)`; `NULL` в уникальности различны, и записи одного
владельца чужую не задевают.
...
`no` — порядковый внутри задачи или проекта, с 1: на него ссылаются (`TRK-42#12`,
`TRK#7`), поэтому он без дыр. Выдаёт его репозиторий под блокировкой строки владельца
(`EntryRepository.allocate_no`, `allocate_project_no`), а не счётчик на владельце:
счётчик терял бы номер на откате, и в деле появлялась бы дыра ровно там, где ссылка
...
class Entry(BaseModel, CreatedByMixin):
    ...
    __table_args__ = (
        ...
        # То же для дела проекта: `TRK#7`. Строки задач держат здесь `NULL` и уникальность
        # не задевают.
        UniqueConstraint("project_id", "no"),
        # Владелец ровно один: запись без владельца не адресовать, запись двух владельцев
        # попала бы в два дела с одним номером.
        CheckConstraint("num_nonnulls(task_id, project_id) = 1", name="one_owner"),
    ...
    # Без `ondelete`: задачи и проекты не удаляются, а если это однажды случится — база
    # откажет, и дело не исчезнет молча. Непуст ровно один из двух (`one_owner`).
