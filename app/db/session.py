...
Граница транзакции описана ровно один раз — в `transaction`. `session_scope` открывает
под неё сессию, `get_session` подаёт то же самое зависимостью FastAPI. Расходиться путям
некуда: HTTP-запрос, вызов инструмента MCP и команда `app.cli` фиксируют изменения одним
и тем же кодом.
...
Здесь же — единственный перевод нарушения целостности в доменную ошибку. Он стоит на
границе, а не в обработчике ошибок HTTP, потому что интерфейсов три: REST переводил бы
своим обработчиком, а MCP и командная строка отдавали бы наружу сырой `IntegrityError` —
крах вызова вместо отказа, по которому агент исправляет запрос.
"""
...
async def transaction(session: AsyncSession) -> AsyncIterator[AsyncSession]:
    """Граница фиксации изменений на готовой сессии: выход без исключения — коммит.

    Отдельно от `session_scope` ради тех, у кого сессия уже есть: тестовая фикстура
    подменяет сессию MCP-сервера своей, откатываемой, и обязана дать сценарию **ту же**
    границу. Своя копия этих шести строк в тестах разошлась бы с боевой на первой же
    правке — и разошлась бы молча, потому что копия зелёная всегда.

    Нарушение целостности переводится в доменный конфликт здесь, а не по месту вставки.
    Создающие сценарии (очередь, участник, токен, связь, запись) устроены как «проверить
    и вставить», и уникальность в базе — не перестраховка, а сама развязка гонки: чей
    `INSERT` пришёл вторым, тот и узнаёт об этом от базы. Наружу такой отказ обязан
    выглядеть одинаково во всех трёх интерфейсах, а `IntegrityError` — это подробность
    драйвера, которую ни агент, ни фронтенд разбирать не должны.

    Момент, в который база сказала «нет», клиенту тоже безразличен: отказ на `flush`
    приходит сюда исключением тела, отказ на отложенном ограничении — из `commit`. Обе
    ветки зовут `integrity_conflict`, поэтому ответ один и тот же.
    """
    try:
        yield session
    except IntegrityError as exc:
...
async def session_scope() -> AsyncIterator[AsyncSession]:
    """Сессия приложения под общей границей транзакции.

    Используется напрямую MCP-сервером и командами `app.cli`; HTTP-запросы получают её
    через `get_session`. Менять поведение транзакции нужно в `transaction` — второго
    такого места в проекте нет.
    """
    async with get_sessionmaker()() as session, transaction(session):
        yield session
...
async def get_session() -> AsyncIterator[AsyncSession]:
    ...
    командной строки, фиксируется одинаково, потому что фиксирует её один и тот же код.
