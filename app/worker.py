...
перезапустить, соединение — оборваться. Ошибка уходит в лог целиком: молчаливый сбой
хуже падения, потому что события копились бы без объяснения.

Целиком — кроме одного случая. Непромигрированная база не поломка, а нормальное
состояние первых секунд контура, и трассировка на каждый круг превращает старт в
простыню, в которой настоящую ошибку никто не найдёт. Такой круг пишет одну строку
предупреждения; всё остальное — по-прежнему трассировкой.
    ...
    try:
        while not stop.is_set():
            try:
                ...
                # неделями. Базу могут перезапустить, соединение — оборваться, всё это
                # временно и лечится ожиданием. Ошибка при этом уходит в лог целиком:
                # молчаливый сбой был бы хуже падения, потому что события копились бы, а
                # причина осталась бы неизвестной. Незафиксированная транзакция
                # откатывается сама, и событие, на котором сбой случился, остаётся
                # необработанным.
                #
                # Исключение одно — непромигрированная база. При первом запуске контура
                # таблиц ещё нет (миграции идут отдельным шагом и позже), это ожидаемое
                # состояние старта, а не поломка, и трассировка каждые пять секунд ровно
                # то, из-за чего настоящую ошибку в логе не замечают.
