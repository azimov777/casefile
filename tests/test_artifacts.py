...
живёт в `ui/docs/FRONTEND.md` вместе со своей проверкой в наборе интерфейса: она
сверяет документ с этими же двумя выгрузками, читая их из корня репозитория.

Почему схема вообще лежит в репозитории. До задачи 29 её намеренно не хранили: вторая
копия отстаёт от первой. Отставание сняли тестом ниже, а взамен интерфейс берёт схему
из git и генерирует клиент, не поднимая Docker.
"""
