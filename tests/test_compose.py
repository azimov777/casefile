...
#: собранному из них `TRACKER_DATABASE_URL`. `ui`, `updater` и `updater-renew` есть только
#: у прод-контура, который стал установкой одной строкой (TRK-58): образ интерфейса,
#: обновлятор установки и замена самого обновлятора (TRK-119), у каждого своё окружение.
FOREIGN_SERVICES = frozenset({"db", "ui", "updater", "updater-renew"})
