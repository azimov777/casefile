...
(завершение спринта на сотню задач) не должна размножаться. Третье: ошибка правила не
...
async def test_a_wave_of_events_does_not_multiply(
    ...
    Завершение спринта даёт по событию `issue.updated` на каждую незакрытую задачу.
