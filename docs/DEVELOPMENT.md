...
Он сливает без коммита, прогоняет на результате слияния весь набор в контейнере и коммитит
только зелёное, оставив в сообщении строку `Merge-verified:` с исходом прогона. Красный
набор — `git merge --abort`: ветка не слита, `main` прежний. Конфликты скрипт оставляет
