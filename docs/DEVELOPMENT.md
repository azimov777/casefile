...
| `local-token` | интерфейсу этой машины | `task` | **только в файл**, права `0600` | ничего не делает |

`local-token` существует потому, что на локальной установке человек у трекера один, и
заставлять его быть администратором ради первого экрана незачем: ключ добывает сама
установка, а человек его не видит и не хранит.
...
хеш, и секрет выпущенного токена не восстановить. Отсюда четыре исхода:

1. в файле действующий секрет — не делается **ничего**, файл не переписывается;
2. установка пуста — заводится владелец и выпускается его первый токен набора `task`;
...
4. названного участника нет, а установка не пуста — отказ `participant_not_found` с кодом
   возврата 1. Второго участника опечатка в имени не заведёт.

Набор — `task`, и это ограничение, а не умолчание: ключ уезжает в браузер, и открывать им
выпуск токенов и правку участников нельзя. Имя токена (`local-ui`) постоянное — по нему же
находится предшественник под отзыв.
...
**Порядок команд при первом запуске имеет значение.** `init` считает установку настроенной,
если в базе есть хотя бы один токен — любой. Поэтому `local-token` на пустой установке
закрывает дверь `init`, и токена набора `main` у человека не появится: его придётся выпустить
явно через `issue-token`. Хотите и то и другое — зовите `init` раньше:

```bash
docker compose run --rm migrate
docker compose run --rm init          # доступ человеку, набор main
docker compose run --rm local-token   # ключ интерфейсу, набор task
```
...
Доступа человеку с набором `main` установка сама не выпускает: `init` печатал бы секрет в
журнал контейнера. Нужен — выпускается явно, секрет печатается в этот терминал:
