...
# Docker-контуром бэкенда, а большинство веток проекта TRK каталог `ui/` не касаются.
# Сквозные `pnpm e2e` в этот скрипт осознанно не добавлены — они остаются за
# `ui/scripts/merge-task-branch.sh`, которым сливают ветки проекта UI.
