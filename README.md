...
В прод-контуре сервиса `local-token` нет: тома с исходниками там нет, и файлу негде лечь
так, чтобы его увидел хост. Локальная установка на одного человека — это дев-контур: его
службы подняты с `restart: unless-stopped` и есть работающая установка трекера (раздел
«Команды разработки»).
...
и описании) и три вычисляемых признака — `blocked`, `open_questions`,
