...
`issue.deleted`, `status.issues_moved`. Словарь открытый — следующие задачи дописывают свои
в `app/domain/events.py`. Тип выводится из действия, которым сценарий проверял права, чтобы
имена действий и имена событий не разъехались.
