...
> фундамент нового построен (задача 21). Поиск и лента строятся задачами 25–29 по
> `docs/CONCEPT.md`, порядок работ — в `docs/ROADMAP.md`, полная переработка этого
> файла — задача 29. Прежняя версия целиком доступна в git по коммиту `49e2e49`.
>
...
> разделами, зашитыми статусами и переходами, дело с записями всех типов и пакетом
> преемника, связи трёх видов с блокировкой входа в работу и незакрытыми детьми,
> оболочка ответа и коды ошибок, выгрузка OpenAPI, оба контура Docker. MCP-сервер
> поднимается и отвечает на `initialize`, но инструментов пока не объявляет — их строит
> задача 28.
