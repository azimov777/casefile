...
Сервис есть в обоих контурах, и файл в обоих ложится в `.secrets/ui-token`: интерфейс
монтирует один и тот же путь, каким бы контуром ни был поднят бэкенд. Наружу файл выходит
по-разному — в разработке репозиторий смонтирован целиком, а в проде код лежит в образе, и
каталог под ключ монтируется разовому сервису отдельно. Рецепт и его особенности — в разделе
«Продакшен-контур».

Владельца файла контуры тоже назначают по-разному, и `cat` из подсказки выше работает
...
нечего: контур по-прежнему поднимается одной командой. Прод-контур решает то же иначе,
переменной `TRACKER_SECRETS_USER`, — там процесс непривилегированный и чужого uid взять
не может. Разбор обоих механизмов — `docs/notes/docker.md`.
...
проверить его больше некому — ни конвейера, ни удалёнки у репозитория нет. Правило целиком
...
mkdir -p .secrets             # каталог под ключ интерфейса; почему руками — ниже
docker compose -f docker-compose.prod.yml up -d --build
docker compose -f docker-compose.prod.yml run --rm migrate
docker compose -f docker-compose.prod.yml run --rm init          # доступ человеку, набор main
docker compose -f docker-compose.prod.yml run --rm local-token   # ключ интерфейсу, набор task
```

Порядок последних двух команд важен ровно так же, как в разработке: `init` считает установку
настроенной по наличию любого токена (раздел «Ключ для локального интерфейса»).
...
| Владелец файлов, записанных в репозиторий | возвращается хозяину каталога командой сервиса | задаётся `user:` из `TRACKER_SECRETS_USER` |
...
### Ключ интерфейса в прод-контуре

`local-token` кладёт секрет туда же, куда в разработке, — `.secrets/ui-token` рядом с
compose-файлом, — и интерфейс монтирует этот путь независимо от того, каким контуром поднят
бэкенд. Устроено это иначе, и разницу видно в самом сервисе:

- каталог `.secrets` монтируется **этому сервису**, а не всем: `api` и `mcp` предъявленный
  токен сверяют по базе, и секрет на диске им не нужен. Отличие «код в образе, тома с
  исходниками нет» этим не нарушено — монтируется каталог под один файл, а не код;
- `user:` называет владельца файла. Права `0600` открывают файл только владельцу, а читают
  его человек с хоста и контур интерфейса; образ же работает от непривилегированного
  `tracker` (uid 1000). Умолчание совпадает с ним; машина, где владелец установки — другой
  пользователь, правит `TRACKER_SECRETS_USER` в `.env`.

Каталог заводится до первого запуска, и `mkdir -p .secrets` в рецепте выше не перестраховка:
недостающий источник bind-mount Docker создаёт сам, на Linux — от root, а непривилегированный
процесс в такой каталог не запишет. На macOS слой обмена файлами Docker Desktop владельца не
переносит, и там команда отработает и без этого шага, — то есть проверка решения на одной
только macOS ничего не говорит о правах (`docs/notes/docker.md`).

Проверить выданный ключ, не печатая его:
