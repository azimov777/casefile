...
> новый построен задачами 21–28 по `docs/CONCEPT.md`. Осталась сборка — задача 29:
> `bootstrap`, демо-данные, справочник ошибок и полная переработка этого файла. Порядок
> работ — в `docs/ROADMAP.md`. Прежняя версия целиком доступна в git по коммиту `49e2e49`.
>
...
> преемника, связи трёх видов с блокировкой входа в работу и незакрытыми детьми, отбор
> задач языком запросов и фильтром, лента журнала с ожиданием и потоком SSE,
> идемпотентность создающих вызовов, оболочка ответа и коды ошибок, выгрузка OpenAPI,
> оба контура Docker и MCP-сервер с двадцатью инструментами, промптом дисциплины и
> инструкциями из скила.
