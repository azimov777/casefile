...
const NOT_IN_CODE: Record<string, string> = {
  ...
  serverUrl: 'поле конфигурации Windsurf: им объяснено, почему JSON-фрагмент не для него',
  httpUrl: 'поле конфигурации Gemini CLI: им объяснено, почему JSON-фрагмент не для него',
  '.mcp.json': 'файл конфигурации Claude Code в проекте человека: его форму повторяет фрагмент',
  serverInfo: 'поле ответа app-server Codex: им объяснено, как проверить подключение без модели',
  CODEX_HOME: 'переменная окружения Codex, не наш код',
  ...
  getScroll: 'функция `axe-core`, которой он решает, прокручивается ли область, не наш код',
...
/**
 * Абсолютный путь: место на чьей-то машине, а не в репозитории.
 *
 * Проверять его не от чего — на другой машине такого пути нет, и проверка, падающая
 * от этого, хуже отсутствующей: её отключат первым же прогоном. По той же причине
 * такая запись не проверяется и текстом: наш корпус про её имена не знает ничего.
 */
...
/**
 * Место в бэкенде: корень репозитория, на уровень выше интерфейса (`../app/...`).
 *
 * Пока бэкенд жил соседним репозиторием, такие указатели не проверялись: на чужой
 * машине его могло не быть. Теперь дерево одно, и файл с символом в нём проверяются
 * так же, как свои. Текст такой записи с корпусом не сверяется: корпус собран из кода
 * интерфейса, а запись говорит о коде бэкенда, и её имён в нём нет.
 */
function inBackend(token: string): boolean {
  return token.startsWith('../');
}
        ...
        for (const segment of segmentsOf(field[1])) {
          ...
          for (const token of leadingPlaces(segment)) {
            ...
            if (outsideRepo(token)) {
              // Кусок пропускается целиком, а не один токен: символ непроверяемого
              // файла иначе искался бы в соседнем нашем.
              skip = true;
              elsewhere ||= beyondRepo(token);
              continue;
            }
            elsewhere ||= inBackend(token);
