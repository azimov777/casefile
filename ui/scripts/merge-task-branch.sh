...
# нечем.
#
# Отличие от бэкенда (`../scripts/merge-task-branch.sh`, `TRK-45`): там один набор
...
# Корень git-репозитория, содержащего ТЕКУЩИЙ каталог — не каталог, где лежит сам файл
# скрипта: его можно вызвать по абсолютному пути из соседнего дерева и ожидать работы
# с репозиторием, в котором сейчас стоят. В монорепозитории (TRK-64) этот корень —
# каталог бэкенда, а `package.json` интерфейса лежит в `ui/`; в прежнем отдельном
# `tracker-ui` корень и был каталогом интерфейса. Каталог `ui/` в корне репозитория,
# если он есть, побеждает; иначе (старая раскладка) интерфейс — сам корень.
TOPLEVEL="$(git rev-parse --show-toplevel)"
if [ -d "$TOPLEVEL/ui" ]; then
    cd "$TOPLEVEL/ui"
else
    cd "$TOPLEVEL"
fi
