...
test.describe('список на узком экране', () => {
  ...
  test('страница не едет вбок, а название держит свою ширину', async ({ page, request }) => {
    ...
    // Страница вширь не едет: таблица на тесном месте раскладывается карточками.
     ...
     * получает: здесь у названия было 66 px, то есть три буквы и многоточие (UI-66).
     * С UI-134 строки на этом месте — карточки той же разметки, и название берёт почти
     * всю ширину карточки. Мерится ячейка названия первой строки: шапка таблицы в
     * карточках есть только у диктора.
     */
    const title = await rows(page).first().locator('td').first().boundingBox();
