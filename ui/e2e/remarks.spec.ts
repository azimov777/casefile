 ...
 * (`../tracker/app/services/demo.py`). Сценарий только читает и потому идёт в обеих
...
test('признак «замечаний» стоит в строке списка и на карточке доски, и по нему отбирают', async ({
  ...
  await expect(row.getByText('замечаний 1')).toBeVisible();
  ...
  await expect(
    page.getByRole('article').filter({ hasText: 'DEMO-1' }).getByText('замечаний 1'),
  ).toBeVisible();
});
