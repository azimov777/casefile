 ...
 * `pages/task/ui/task-header.tsx`, `entities/entry/ui/entry-headline.tsx`), и здесь
 * это проверено вычисленным стилем, тем же замером, что защищает markdown.
 *
...
test('ключ задачи вне markdown несёт `white-space: nowrap` — опись, таблица, карточка (UI-151)', async ({
  page,
  request,
}) => {
  test.setTimeout(60_000);
