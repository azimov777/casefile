...
/**
 * Второй экземпляр интерфейса — установка в режиме входа по учётным записям (`TRK-113`).
 *
 ...
 * у основного экземпляра (в режиме входа его не отдают никому, `docker/config-json.sh`).
 * Бэкенд общий: учётные записи знает API всего контура.
 *
