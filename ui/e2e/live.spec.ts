...
test('два вопроса подряд видны оба: второй не затирает первый', async ({ page, request }) => {
  ...
  /*
   * Замер контраста снимается в покое. Карточка приходит движением, и на его
   * промежуточных кадрах прозрачность честно роняет контраст ниже AA (замерено:
   * 4.45 при норме 4.5). Это не дефект интерфейса, а неудачная синхронизация
   * замера: WCAG 1.4.3 говорит о предъявленном тексте, а не о кадре, на котором
   * элемент ещё едет (`UI-59#4`). Без ожидания проверка проходит через раз.
   */
