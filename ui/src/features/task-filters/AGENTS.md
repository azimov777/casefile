...
Отбор задач: условия, порядок и курсор как состояние адреса страницы. Своего состояния
...
- `index.ts` — публичный интерфейс среза: `TaskFiltersForm`, `useTaskFilters`,
  ...
  `OPEN_QUESTIONS_CONDITION`, типы `TaskFilters`, `TaskFiltersControl`, `QueryProblem`
