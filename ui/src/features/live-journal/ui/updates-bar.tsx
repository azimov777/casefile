...
export function UpdatesBar({ reading }: UpdatesBarProps) {
   ...
   * держит узел, пока место схлопывается, но не саму полосу: уходящая невидима с того
   * же кадра, что и прежде, — держится только её место. Ссылка задержки стоит на месте:
   * узел снимается, когда его движение кончилось, а не по сроку от нажатия (UI-111).
   *
  ...
  const { item: said, leaving, entering, ref } = held;
