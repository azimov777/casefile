 ...
 * Где это место, решает не стопка, а низ области содержания (`FloatDock`): стопка
 * делит его с полосой обновлений таблицы.
 *
...
export function QuestionNotice({
  ...
  return (
    ...
    <aside
       ...
       * сдвинуть то, что человек читает. Место при этом своё и постоянное — у правого
       * края низа области содержания, где его не закрывает ни шапка, ни первые строки
       * таблицы, которые и читают в первую очередь. Этот низ стопка делит с полосой
       * обновлений и ставит её `FloatDock` (`float-dock.tsx`): рядом с полосой, когда
       * помещаются, и строкой выше, когда нет. Предел ширины общий с полосой.
       *
       * Место под стопкой указатель пропускает насквозь, сама стопка — нет.
       *
       ...
       */
      className="pointer-events-auto flex max-w-(--ui-float-max) flex-col"
    ...
    >
      {held.map(({ key, item: question, leaving, entering, ref }) => (
        <div
          ...
          className={cn(
            /*
             * Карточка держится нижнего края своего места на всём ходу: место
             * открывается и закрывается сверху, соседи съезжают, а сама она стоит —
             * и движется только своим движением (`animate-appear`, `animate-disappear`).
             *
             * Одного `items-end` для этого мало — нужен и `content-end`. Chrome
             * считает долю `fr` дважды: высоту месту — по содержимому (доля от высоты
             * карточки), а строку — ещё раз, уже от этой высоты. Строка выходит короче
             * места, и без `content-end` стоит у его верха: посреди хода карточка
             * всплывала бы над низом места на f(1−f) своей высоты — до четверти на
             * середине — и съезжала обратно (UI-110).
             */
        ...
        >
          {/*
            ...
            Обрезанием (`overflow: hidden`) этого добиваться нельзя: оно срезало бы
            тень карточки и не пустило бы её уехать за край места.
