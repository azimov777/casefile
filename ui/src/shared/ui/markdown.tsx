...
export function Markdown({ children }: { children: string }) {
  return (
     ...
     *
     * `wrap-anywhere` (`overflow-wrap: anywhere`) стоит здесь одна на всю разметку, а не
     * у каждого узла по отдельности: свойство наследуется, и узел-ссылка или ячейка
     * таблицы, которых `components` не перечисляет явно (`Anchor` внутри `td`, GFM-таблица
     * целиком), получают его тем же правилом. Без него длинное слово или код без пробелов
     * (адрес, хеш, путь к файлу) не находят точки переноса и растягивают карточку записи,
     * а с ней страницу вбок на телефоне (UI-150) — `min-content` абзаца равен длине
     * слова целиком, ровно как у названия задачи (`entities/task/ui/task-card.tsx`).
