...
/*
 * Варианты собраны `cva` по образцу кнопки, но склейки `cn` здесь нет намеренно:
 * `twMerge` считает `text-label` и `text-neutral` одной группой «text-*» и оставляет
 * из них последний — кегль плашки молча пропал бы, а размер она унаследовала бы от
 * строки таблицы. Внешнего `className` плашка не принимает, и мирить с ним нечего.
 */
const badge = cva(
  [
    /*
