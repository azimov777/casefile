 ...
 *
 * По этому же правилу `waiting` — единственный статус тона `attention`: он означает
 * «ход не за агентом», а ход, которого ждут, — человеческий. Это ровно то положение
 * дел, которое меняет решение смотрящего, и потому янтарный здесь не украшение.
 * Нейтральным его красить нельзя: серый сказал бы «обычный ход дел», а ожидание
 * человека обычным ходом дел не является.
 */
