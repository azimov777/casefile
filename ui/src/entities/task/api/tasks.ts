 ...
 *
 * `parent` — родитель строки, ключ и название (TRK-95; одним значением с TRK-135): им
 * карточка и строка подписывают, из какой программы задача (UI-119). Родители всей
 * страницы приезжают этим же запросом, поэтому запроса на карточку родителя нет. Без
