...
function BoardColumn({ status, params, explained, open, onToggle }: BoardColumnProps) {
  ...
  return (
    ...
    >
      {/*
       * Липкий заголовок (UI-69): на любой глубине прокрутки видно, какой столбец
       * перед глазами. Липкость включается там же, где своя прокрутка столбца, —
       * с точки остановки `fold`, и это не украшение симметрии, а единственная
       * ветка, в которой заголовку есть к чему липнуть. Выше `fold` ближайший порт
       * прокрутки заголовка — сам столбец (`fold:overflow-y-auto`, UI-68), и он же
       * уезжает; ниже `fold` столбец прокручиваемой областью не является, а порт
       * прокрутки всё равно не окно, а **ряд столбцов**: `overflow-x-auto` делает
       * его прокручиваемым по обеим осям (`docs/notes/ui.md`, «Липкую шапку таблицы
       * ломает `overflow` у её обёртки»). Ряд по вертикали не прокручивается вовсе,
       * поэтому `sticky` там ничего не делает: замерено — страница уехала на 270 px,
       * заголовок уехал вместе со своим столбцом (`top` −23 при `top` столбца −24).
       * Оставлять липкость ниже `fold` значило бы обещать поведение, которого нет.
       *
       * `top-0` без всякой поправки: над заголовком не стоит ничего — ни шапки
       ...
       * 2. Свой фон: у прилипшего элемента фона нет, и карточки просвечивают сквозь
       *    него. Фон тот же, что у столбца (`bg-sunken`), — заголовок обязан читаться
       *    продолжением своего столбца, а не плашкой поверх него.
       * 3. `z-2`, а не `z-1`: карточка позиционирована (растянутая ссылка), и
       *    позиционированное рисуется поверх липкого по порядку разметки, а карточка
       *    в разметке ниже. Единицы мало: внутри карточки свои поднятые узлы —
       *    название и исполнитель стоят `relative z-1` над растяжкой
       *    (`entities/task/ui/task-card.tsx`), и при равном `z-index` побеждает опять
       *    тот, кто ниже, — то есть проезжающая карточка поверх шапки.
       *
      ...
      <h3
        className={cn(
          '-mx-3 rounded-t-control px-3 pt-3 pb-2 text-body fold:sticky fold:top-0 fold:z-2',
          !reveal.held || empty ? 'bg-transparent' : 'bg-sunken fold:shadow-sticky',
        )}
      >
        <button
          type="button"
