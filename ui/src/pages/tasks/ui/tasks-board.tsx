...
function BoardColumn({ status, params, explained, open, onToggle }: BoardColumnProps) {
  ...
  return (
    <section
       ...
       */
      className={cn(
        'relative flex w-(--ui-board-column) shrink-0 basis-(--ui-board-column) flex-col rounded-control border border-line bg-sunken px-3 pb-3 fold:min-h-0 fold:overflow-y-auto',
        /*
    ...
    >
       ...
       *
       * 1. Верхнего поля у столбца больше нет (`px-3 pb-3` вместо `p-3`), а свои
       *    двенадцать пикселей заголовок носит сам (`pt-3`). Липкое внутри
       *    прокручиваемого столбца считается от его **содержимого**, а не от рамки:
       *    пока поле стояло на секции, `top-0` прижимал заголовок к нижнему краю
       *    поля — на тринадцатом пикселе от рамки вместо первого (замерено: `top`
       *    заголовка 133 при `top` столбца 120), — и над ним оставалась щель
       *    в двенадцать пикселей, сквозь которую ехали карточки. Отрицательное поле
       *    заголовку не помогало: `sticky` возвращал его обратно вниз, к своей
       *    границе. `-mx-3` при своих `px-3` остаётся — вбок заголовок обязан
       *    доставать до рамки, чтобы карточка не просвечивала краем.
